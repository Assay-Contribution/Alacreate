...
const submitButton = document.querySelector<HTMLButtonElement>("#reportSubmitButton");
const message = document.querySelector<HTMLDivElement>("#reportMessage");

if (reportForm && reportDate && taskList && addTaskButton && submitButton && message) {
  ...
  const addTask = (task: ReportTask = { title: "", minutes: 30 }) => {
    ...
    `;
    taskRow.querySelector<HTMLButtonElement>(".remove-task")?.addEventListener("click", () => {
      taskRow.remove();
      syncTaskControls();
    });
  ...
  reportForm.addEventListener("submit", async (event) => {
    ...
    const tasks = Array.from(taskList.querySelectorAll<HTMLDivElement>(".task-row")).map((row) => ({
      title: row.querySelector<HTMLInputElement>(".task-title")?.value.trim() ?? "",
      minutes: Number(row.querySelector<HTMLInputElement>(".task-minutes")?.value ?? 0),
    }));

    if (tasks.length < 3 || tasks.some((task) => !task.title || task.minutes < 1)) {
    ...
    if (error) {
      showError("We couldn't save this report. Please try again.", submitButton, message);
      return;
    }
...
function showError(
  ...
  submitButton.disabled = false;
  submitButton.innerHTML = 'Save today\'s report <span aria-hidden="true">↗</span>';
